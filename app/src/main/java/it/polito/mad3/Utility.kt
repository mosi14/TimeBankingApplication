...
import it.polito.mad3.chat.ChatChannel
...
val chatChannelsCollectionRef = firestoreInstance.collection("chatChannels")
...
object Utility {
        ...
        insertQuery.get().addOnCompleteListener { insertTask ->
            if (insertTask.isSuccessful) {
                ...
                // if a document with current timeSlot found in the DB
                    ...
                    val updateQuery =
                        db.collection("TimeSlots").whereEqualTo("id" , timeSlotItem.id)
                    updateQuery.get()
    ...
    fun sendInterestNotification(itUser : ProfileData , timeSlotItem : TimeSlotItem) {
        ...
        val query =
            db.collection("users").whereEqualTo("timeSlotId" , timeSlotItem.id)
                .whereEqualTo("userId" , itUser.id)
        query.get()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    if (task.result == null || task.result?.documents?.size !! == 0) {
            ...
            .addSnapshotListener { value , error ->
                ...
                if (value != null) {
                    ...
                    if (isMine) {
                        ...
                        ratingViewModel.setRatingListAsDriver(
                                ratingList.filter { rating -> rating.reciverFlag == "0" }
                                    .toMutableList())
                        // if flag is 1 , the receiver is Passenger
                        ratingViewModel.setRatingListAsPassenger(
                                ratingList.filter { rating -> rating.reciverFlag == "1" }
                                    .toMutableList())
                    } else {
                        // if flag is 0 , the receiver is Driver
                        ratingViewModel.setRatingListAsDriverOfOtherUsers(
                                ratingList.filter { rating -> rating.reciverFlag == "0" }
                                    .toMutableList())
                        // if flag is 1 , the receiver is Passenger
                        ratingViewModel.setRatingListAsPassengerOfOtherUsers(
                                ratingList.filter { rating -> rating.reciverFlag == "1" }
                                    .toMutableList())
                    }
                }
    ...
    fun updateBookingState(
        ...
        val query =
            db.collection("Booking").whereEqualTo("timeSlotId" , timeSlotItem.id).whereEqualTo(
                    "userId" ,
                    interestedPerson.id
                                                                                              )
        query.get().addOnCompleteListener { task ->
